let shopItemsMasterList = {
    ...
    'firecracker':new ShopItem('firecracker','firecracker','item','(item: deal 20 dmg, lose 2 hp)', 5, 0, function(){
         g.player.inv.item.push(new Item('firecracker', 'firecracker', 'combat-enemy', 'During combat, use to deal 20 damage (bypasses armor) to the current enemy, lose 2 hp', 1, 1, function(){
            g.cEnemy.hp-=20;
             g.player.hp-=2;
        }));
    },'common'),
    'throwingegg':new ShopItem('throwing eggs','throwingegg','item','(3x item: current enemy loses 3 armor)', 7, 0, function(){
         g.player.inv.item.push(new Item('throwing eggs', 'throwingegg', 'combat-enemy', "During combat, lower the current enemy's armor by 3", 3, 1, function(){
            g.cEnemy.arm-=3;
        }));
    },'common'),
    'bandages':new ShopItem('bandages','bandages','item','(4x item: heal 5 hp)', 9, 0, function(){
         g.player.inv.item.push(new Item('bandage', 'bandage', 'all-self', "Heal 5", 4, 1, function(){
             g.player.gainHp(5);
        }));
    },'common'),
    'firstaidkit':new ShopItem('first aid kit','firstaidkit','item','(item: heal 20)', 8, 0, function(){
         g.player.inv.item.push(new Item('first aid kit', 'firstaidkit', 'all-self', "Heal 20", 1, 1, function(){
             g.player.gainHp(20);
        }));
    },'common'),
   'sharpeningstone':new ShopItem('sharpening stone','sharpeningstone','item','(3x item: +2 dmg on weapon)', 26, 0, function(){
         g.player.inv.item.push(new Item('sharpening stone', 'sharpeningstone', 'all-self', "Your currently equipped weapon gains +2 dmg", 3, 1, function(){
